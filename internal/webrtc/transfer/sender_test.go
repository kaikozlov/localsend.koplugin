...
// TestRTCSender_ReceiverTokenStorage verifies that the receiver's token is stored
// for PAIR verification.
func TestRTCSender_ReceiverTokenStorage(t *testing.T) {
	sender := NewRTCSender(nil, nil, "")

	// Initially empty
	if sender.receiverToken != "" {
		t.Errorf("receiverToken should initially be empty, got %q", sender.receiverToken)
	}

	// Simulate storing a token (what handleTokenResponse does)
	testToken := "sha256.abc123.def456.ed25519.sig789"
	sender.receiverToken = testToken

	if sender.receiverToken != testToken {
		t.Errorf("receiverToken = %q; want %q", sender.receiverToken, testToken)
	}
}

// TestRTCSender_PairVerificationSuccess verifies that token verification
// succeeds when the PAIR public key matches the token's signer.
func TestRTCSender_PairVerificationSuccess(t *testing.T) {
	// Create receiver signing key
	...
	// Verify the token against the public key - THIS IS WHAT handleFileAcceptance DOES
	err = crypto.VerifyTokenNonce(parsedKey, receiverToken, combinedNonce)
	if err != nil {
		t.Errorf("Token verification should succeed with matching key: %v", err)
	}
}

// TestRTCSender_PairVerificationFailure verifies that token verification
// fails when the PAIR public key does NOT match the token's signer.
func TestRTCSender_PairVerificationFailure(t *testing.T) {
	// Create two different keys
	realReceiverKey, err := crypto.GenerateKeyPair()
	if err != nil {
		t.Fatalf("Failed to generate real receiver key: %v", err)
	}
	fakeReceiverKey, err := crypto.GenerateKeyPair()
	if err != nil {
		t.Fatalf("Failed to generate fake receiver key: %v", err)
	}
	...
	// Generate token with REAL receiver's key
	receiverToken, err := realReceiverKey.GenerateTokenWithNonce(combinedNonce)
	if err != nil {
		t.Fatalf("Failed to generate receiver token: %v", err)
	}

	// Get FAKE receiver's public key PEM (simulating an attack)
	fakeReceiverPublicPEM := fakeReceiverKey.PublicKeyPEM()

	// Parse the fake public key
	parsedKey, err := crypto.ParsePublicKeyPEM(fakeReceiverPublicPEM)
	if err != nil {
		t.Fatalf("Failed to parse fake receiver public key: %v", err)
	}
